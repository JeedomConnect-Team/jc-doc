...
Cette configuration se trouve sur la page `Gestion` du plugin JeedomConnect, bouton **Services de streaming** (section TURN), et peut aussi être gérée directement depuis l'application mobile : **Préférences &gt; Caméras hors LAN**.
...
| **Payant - abonnement géré** | ~6€/an | Aucun (achat intégré App Store / Google Play) | Essai : 4 Go/7 jours sans CB, puis 10 Go/mois |

Le choix se fait via le sélecteur **Mode**, disponible aussi bien dans la modale **Services de streaming** du plugin (page Gestion) que dans l'application (Préférences &gt; Caméras hors LAN).
...
Un bouton **Tester l'accès** permet de vérifier que tout est correctement configuré avant de l'utiliser en conditions réelles.
...
Passé l'essai (ou pour s'abonner directement), un abonnement à **~6€/an** donne accès à **10 Go/mois**, sans limite de durée. Contrairement à l'essai gratuit (qui se démarre depuis le plugin), l'abonnement s'achète et se gère **depuis l'application mobile** : **Préférences &gt; Caméras hors LAN &gt; S'abonner** - un achat intégré classique (App Store sur iOS, Google Play sur Android), sans clé à saisir nulle part. L'abonnement est rattaché à votre installation Jeedom (pas à votre téléphone) : une fois souscrit, tous les appareils connectés à cette même box en bénéficient.

Si vous changez de téléphone ou réinstallez l'application, le bouton **Restaurer mes achats** (même écran) permet de récupérer un abonnement déjà payé sans repayer.

:::tip
Ce même abonnement augmente aussi le quota de widgets [Web View accessibles hors LAN via tunnel](./webviewTunnel.md) (1 → 10 widgets) - même s'il est présenté ici pour les caméras.
:::

Un bouton **Tester l'accès** (ou l'essai en cours) permet de vérifier à tout moment que tout fonctionne - disponible à la fois dans le plugin et dans l'application.

### Suivi de consommation

Une fois l'essai démarré ou l'abonnement actif, un encart affiche en temps réel (dans le plugin comme dans l'application) :
...
### Quota dépassé ou abonnement inactif

Si le quota est atteint, si l'essai est terminé, ou si l'abonnement n'est plus actif (résilié, expiré, ou paiement refusé), l'accès au mode payant est **automatiquement bloqué** : les caméras hors LAN ne pourront plus se connecter via ce relais tant que la situation n'est pas corrigée (renouvellement, nouvel abonnement, ou attente du mois suivant pour un abonnement actif).
...
go2rtc (le composant qui gère les flux vidéo) n'est pas le démon principal du plugin et n'apparaît donc pas sur la page équipement avec les contrôles habituels de démon Jeedom. Un bouton **Redémarrer go2rtc** - disponible à la fois à côté du sélecteur de **Mode** dans le plugin, et dans l'écran Caméras hors LAN de l'application - permet de le relancer manuellement, notamment nécessaire après un changement de mode TURN pour que le nouveau relais soit pris en compte.
